...
class Viewport3D(QWidget):
    ...
    def _make_cylinder_mesh(cx: float, cy: float, z_bottom: float,
        ...
        """Generate a truncated cone as triangle faces."""
        ...
        bot_x = cx + bot_r * np.cos(theta[:-1])

        bot_y = cy + bot_r * np.sin(theta[:-1])

        bot_z = np.full_like(bot_x, z_bottom)

        bottom_verts = np.stack([bot_x, bot_y, bot_z], axis=1)



        top_x = cx + top_r * np.cos(theta[:-1])

        top_y = cy + top_r * np.sin(theta[:-1])

        top_z = np.full_like(top_x, z_bottom + height)

        top_verts = np.stack([top_x, top_y, top_z], axis=1)
        ...
        for i in range(sides):
            ...
            faces.append(np.array([
                ...
                bottom_verts[i], bottom_verts[next_i], top_verts[i]

            ]))

            faces.append(np.array([

                top_verts[i], bottom_verts[next_i], top_verts[next_i]
